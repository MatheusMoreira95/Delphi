...
uses
  ...
  ZAbstractRODataset, ZAbstractDataset, ZDataset , Vcl.Forms , Vcl.Buttons,
              ...
              if TBitBtn(aForm.Components[I]).Tag = 99 then
                begin
                  oAcaoAcesso.descricao :=' - BOTÃO '+
                   StringReplace(TBitBtn(aForm.Components[I]).Caption, '&','',[rfReplaceAll]);
...
begin
  try
    ...
    QryAcaoAcesso.Open ;
     ...
     qry.First;
      while not Qry.Eof do     //usarios
      begin
        QryAcaoAcesso.First;

        while not QryAcaoAcesso.Eof do       //açãoAcesso
        begin
